"""
AI-generated Arabic recommendations and summary for an IEEE check report.

Feeds only the aggregated statistics (not raw paper text) to the local LLM
to keep the prompt small and inference fast, and asks it to write natural,
paper-specific Arabic prose instead of filling in a fixed template. Falls
back to the original template-based ``generate_recommendations`` /
``generate_summary`` if the model is unavailable or fails.
"""
...
def generate_recommendations_and_summary_ai(result: IEEECheckResult) -> Tuple[str, List[str]]:
    """Return (summary, recommendations) generated by the local LLM, with template fallback."""
    try:
